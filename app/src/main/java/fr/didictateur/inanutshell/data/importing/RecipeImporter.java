package fr.didictateur.inanutshell.data.importing;
...
public class RecipeImporter {
        ...
        executorService.execute(() -> {
            try {
                ...
                if (recipe != null) {
                    ...
                    // // recipe.setSource(url); // Méthode non disponible dans Recipe
    ...
    private Recipe parseRecipeFromJsonLd(JSONObject json) throws JSONException {
        ...
        if (!cookTime.isEmpty()) {
            recipe.setCookTime(String.valueOf(parseDuration(cookTime)));
        }
        if (!prepTime.isEmpty()) {
            recipe.setPrepTime(String.valueOf(parseDuration(prepTime)));
        }
        if (!totalTime.isEmpty() && (recipe.getTotalTime() == null || recipe.getTotalTime().isEmpty())) {
            recipe.setTotalTime(String.valueOf(parseDuration(totalTime)));
        }
        ...
        if (json.has("recipeYield")) {
            ...
            try {
                recipe.setRecipeYield(String.valueOf(Integer.parseInt(yield.replaceAll("[^0-9]", ""))));
            } catch (NumberFormatException e) {
                recipe.setRecipeYield(String.valueOf(4));
            }
        }
    ...
    private Recipe parseMicrodata(Document doc) {
        ...
        if (recipeElements.size() > 0) {
            ...
            if (cookTimeEl != null) {
                recipe.setCookTime(String.valueOf(parseDuration(cookTimeEl.attr("datetime"))));
            }
    ...
    private Recipe parseMealieJson(JSONObject json) throws JSONException {
        ...
        recipe.setName(json.optString("name", "Recette importée"));
        recipe.setDescription(json.optString("description", ""));
        recipe.setCookTime(String.valueOf(json.optInt("cookTime", 0)));
        recipe.setPrepTime(String.valueOf(json.optInt("prepTime", 0)));
        recipe.setTotalTime(String.valueOf(json.optInt("totalTime", 0)));
        recipe.setRecipeYield(String.valueOf(json.optInt("servings", 4)));
